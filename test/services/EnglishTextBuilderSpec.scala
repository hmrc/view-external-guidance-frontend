...
class EnglishTextBuilderSpec extends BaseSpec {
  ...
  "TextBuilder placeholder parsing" must {
    ...
    "Convert label reference with default output format placeholders within phrase to Bold expanded TextItems" in new Test {
      ...
      TextBuilder.fromPhraseWithOptionalHint(expanded)._1.items shouldBe List(Words("Sentence containing ") , Words("1.0", true) ,  Words(" (label reference)"))
    }
    ...
    "Obtain a length of list value for a given list label" in new Test {
      ...
      val expanded = TextBuilder.expandLabels(p, ctx.labels)
      TextBuilder.fromPhrase(expanded).items shouldBe Seq(Words("The length of the colours list is 3 (label reference)"))
    ...
    "Convert label reference with currency output format placeholders within phrase to expanded TextItems" in new Test {
      ...
      val expanded = TextBuilder.expandLabels(p, ctx.labels)
      TextBuilder.fromPhrase(expanded).items shouldBe Seq(Words("Sentence containing £1.00 (label reference)"))
    ...
    "Convert label reference with date output format placeholders within phrase to expanded TextItems" in new Test {
      ...
      TextBuilder.fromPhraseWithOptionalHint(expanded)._1.items shouldBe Seq(Words("Sentence with a 22 September 1973 label reference"))
    }
    ...
    "Convert a label placeholder within a bold placeholder to a bold label ref" in new Test {
      ...
      val expanded = TextBuilder.expandLabels(p, ctx.labels)
      TextBuilder.fromPhrase(expanded).items shouldBe Seq(Words("Sentence with a "), Words("1.0", true), Words(" label reference"))
      TextBuilder.fromPhraseWithOptionalHint(expanded)._1.items shouldBe Seq(Words("Sentence with a "), Words("1.0", true), Words(" label reference"))
    }
    ...
    "Convert a label placeholderwith currency output format within a bold placeholder to a bold label ref" in new Test {
      ...
      val expanded = TextBuilder.expandLabels(p, ctx.labels)
      TextBuilder.fromPhrase(expanded).items shouldBe Seq(Words("Sentence with a "), Words("£1", true), Words(" label reference"))
      TextBuilder.fromPhraseWithOptionalHint(expanded)._1.items shouldBe Seq(Words("Sentence with a "), Words("£1", true), Words(" label reference"))
    }
    ...
    "Convert [list:<label>:length] placeholders within phrase to Words TextItems containing the list label length" in new Test {
      ...
      val expanded = TextBuilder.expandLabels(p, ctx.labels)
      TextBuilder.fromPhrase(expanded).items shouldBe Seq(Words("The number of colours is 3"))
      TextBuilder.fromPhraseWithOptionalHint(expanded)._1.items shouldBe Seq(Words("The number of colours is 3"))
    }
    ...
    "Convert [list:<label>:length] placeholders within phrase when list does not exist" in new Test {
      ...
      val expanded = TextBuilder.expandLabels(p, ctx.labels)
      TextBuilder.fromPhrase(expanded).items shouldBe Seq(Words("The number of colours is "))
      TextBuilder.fromPhraseWithOptionalHint(expanded)._1.items shouldBe Seq(Words("The number of colours is "))
    }
  ...
  "TextBuilder expandLabels" must {
    ...
    "Convert label reference with default output format placeholders within phrase" in new ExpandTest {
      ...
      val expectedPhrase = Phrase("""Sentence with a 33.5 label reference""", """Welsh: Sentence with a [label:BLAH] label reference""")
    ...
    "Convert label reference with currency output format placeholders within phrase" in new ExpandTest {
      ...
      val expectedPhrase = Phrase("""Sentence with a £33.50 label reference""", """Welsh: Sentence with a [label:BLAH:currency] label reference""")
      TextBuilder.expandLabels(phrase, labels) shouldBe expectedPhrase
    }
    ...
    "Convert label reference with date output format placeholders within phrase" in new ExpandTest {
      ...
      val expectedPhrase = Phrase("""Sentence with a 22 September 1973 label reference""", """Welsh: Sentence with a [label:SomeDate:date] label reference""")
      TextBuilder.expandLabels(phrase, labels) shouldBe expectedPhrase
    }
    ...
    "Convert a label placeholder within a bold placeholder to a bold label ref" in new ExpandTest {
      ...
      val expectedPhrase = Phrase("""Sentence with a [bold:33.5] label reference""", """Welsh: Sentence with a [bold:[label:BLAH]] label reference""")
      TextBuilder.expandLabels(phrase, labels) shouldBe expectedPhrase
    }
    ...
    "Convert a label placeholder with currency output format within a bold placeholder" in new ExpandTest {
      val phrase = Phrase("""Sentence with a [bold:[label:BLAH:currencyPoundsOnly]] label reference""",
        """Welsh: Sentence with a [bold:[label:BLAH:currencyPoundsOnly]] label reference""")
      val expectedPhrase = Phrase("""Sentence with a [bold:£33] label reference""", """Welsh: Sentence with a [bold:[label:BLAH:currencyPoundsOnly]] label reference""")
      TextBuilder.expandLabels(phrase, labels) shouldBe expectedPhrase
    }
    ...
    "Convert a list length placeholder " in new ExpandTest {
      ...
      val expectedPhrase = Phrase("""SomeList has length 3""", """Welsh: SomeList has length [list:SomeList:length]""")
      TextBuilder.expandLabels(phrase, labels) shouldBe expectedPhrase
    }
    ...
    "Convert a list index placeholder using first" in new ExpandTest {
      ...
      TextBuilder.expandLabels(phrase, labels).english shouldBe expectedPhrase.english
    }
    ...
    "Convert a dateplacholder inside a phrase into a date" in new ExpandTest {
      ...
      val expectedPhrase = Phrase("""Some sentence with a date Sunday""", """Welsh: Some sentence with a date [date:12/12/2021:dow_name]""")
      TextBuilder.expandLabels(phrase, labels) shouldBe expectedPhrase
    }
    ...
    "Convert a dateplacholder inside a phrase into a date label" in new ExpandTest {
      ...
      val expectedPhrase = Phrase("""Some sentence with a date Saturday""", """Welsh: Some sentence with a date [date:[label:SomeDate]:dow_name]""")
      TextBuilder.expandLabels(phrase, labels) shouldBe expectedPhrase
    }
