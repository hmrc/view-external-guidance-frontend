...
trait SessionRepository extends SessionRepositoryConstants {
  ...
  def updateAfterStandardPage(key: String, processCode: String, labels: Labels, revertOperations: List[LabelOperation], requestId: Option[String]): Future[RequestOutcome[Unit]]
  def updateAfterFormSubmission(key: String, processCode: String, answerId: String, answer: String, labels: Labels, nextLegalPageIds: List[String], revertOperations: List[LabelOperation],
...
class DefaultSessionRepository @Inject() (config: AppConfig, component: MongoComponent)(implicit ec: ExecutionContext)
  ...
  def updateAfterFormSubmission( key: String,
                                 ...
                                 revertOperations: List[LabelOperation],
                                 requestId: Option[String]): Future[RequestOutcome[Unit]] =
    collection.findOneAndUpdate(
