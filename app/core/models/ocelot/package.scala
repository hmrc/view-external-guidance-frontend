...
package object ocelot {
  ...
  val JavascriptPrintPattern: String = "Javascript\\:window\\.print\\(\\)"
  ...
  val linkPattern: String = s"\\[(button|link)(-same|-tab)?:(.+?):($StanzaIdPattern|$HttpUriPattern|$JavascriptPrintPattern)\\]"
